@echo off
rem run_pose_test.bat -- compile and run engine\two_handed_pose_test.cpp against the shipped header.
...
if not exist "%TEMP%\ashes_pose_test" mkdir "%TEMP%\ashes_pose_test"
cl /nologo /EHsc /W3 /std:c++17 /Fo"%TEMP%\ashes_pose_test\\" /Fe"%TEMP%\ashes_pose_test\two_handed_pose_test.exe" two_handed_pose_test.cpp
if errorlevel 1 exit /b 1
"%TEMP%\ashes_pose_test\two_handed_pose_test.exe"
